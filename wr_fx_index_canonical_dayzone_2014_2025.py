...
def fetch_tv_calendar(years):
    """Fetch in <=35-day chunks, select exact US CPI/NFP/Fed decision anchors."""
    wanted={'CPI':'CPI','Non Farm Payrolls':'NFP','Fed Interest Rate Decision':'FOMC'}
    raw=[]
    start=datetime(min(years),1,1,tzinfo=timezone.utc)
    end=datetime(max(years)+1,1,1,tzinfo=timezone.utc)
    cur=start
    while cur<end:
        nxt=min(cur+timedelta(days=35),end)
        p={'from':iso_z(cur),'to':iso_z(nxt-timedelta(milliseconds=1)),'countries':'US'}
        r=requests.get(TV_CAL,params=p,headers=UA,timeout=45); r.raise_for_status()
        payload=r.json()
        if payload.get('status') not in ('ok','success',None):
            raise RuntimeError(f'TV_CAL_STATUS {payload.get("status")}')
        raw.extend(payload.get('result',[]))
        cur=nxt
    selected={}
    for x in raw:
        typ=wanted.get(x.get('title'))
        if not typ: continue
        ds=x.get('date')
        if not ds: continue
    ...
    audit={}
    for y in years:
        e=[x for x in events if x['dt'].year==y]
        counts={k:sum(x['type']==k for x in e) for k in ('CPI','NFP','FOMC')}
        # CPI and NFP should be monthly. FOMC allows extraordinary decisions (notably crisis periods).
        if counts['CPI']!=12 or counts['NFP']!=12 or not (6<=counts['FOMC']<=12):
    ...
    # Official-source spot audit: 2024 BLS/Fed dates checked before this run.
    expected_2024={
      ('NFP','2024-01-05T13:30:00+00:00'),
      ('CPI','2024-01-11T13:30:00+00:00'),
      ('FOMC','2024-01-31T19:00:00+00:00'),
    }
    actual={(x['type'],x['dt'].isoformat()) for x in events}
    miss=expected_2024-actual
...
def group_summary(trades,symbols):
    ...
    for lab in ('T-2','T-1','T0','T+1','T+2','T+3','OFF'):
        by_t[lab]=stats([t for t in trades if t['t_label']==lab and t['zone_ok'] and not t['news_guarded']])
    matrix={}
    for z in ('A','B','C'):
        matrix[z]={lab:stats([t for t in trades if t['zone']==z and t['t_label']==lab and t['zone_ok'] and not t['news_guarded']]) for lab in ('T-2','T-1','T0','T+1','T+2','T+3')}
    sym={s:stats([t for t in full if t['symbol']==s]) for s in symbols}
    non=[v for v in sym.values() if v['n']>0]
    pre22=stats([t for t in full if t['year']<=2021]); post22=stats([t for t in full if t['year']>=2022])
    return {
      'all_time_baseline':stats(trades),'canonical_full':stats(full),'pre2022':pre22,'y2022_2025':post22,
      ...
      'guarded_count':sum(t['news_guarded'] for t in trades),'zone_a_ineligible_count':sum(t['zone']=='A' and not t['zone_ok'] for t in trades),
    }
...
def main():
    ...
    out={'schema':'wr-canonical-dayzone-fx-index-2018-2025-v2','status':'FROZEN_CANONICAL_T_DAY_ZONE_DISCOVERY','strategy':'Wave Rider v2.5.13 frozen core/lifecycle Python replication','group':group,'symbols':symbols,'tf':TF,'years':list(YEARS),'untouched_holdout':'2014-2017 deliberately unopened in this path','canonical':{'event_types':['CPI','NFP','FOMC / Fed Interest Rate Decision'],'t_on':['T-2','T-1','T0','T+2','T+3'],'t_off':['T+1','T+4+ / outside event window'],'zone_A':'02:00-08:00 VN only after prior major event in same 08:00-anchored trading day','zone_B':'16:00-19:00 VN','zone_C':'23:00-02:00 VN','other_time':'OFF','trading_day_anchor':'08:00 Asia/Ho_Chi_Minh','news_guard':'±15m around major event','overlap_precedence':'T0 overrides; otherwise any T+1 state is OFF; remaining allowed overlaps are ON'},'calendar':{'machine_source':'TradingView Economic Calendar historical endpoint','official_spot_audit':'2024 NFP/CPI/FOMC anchor dates were cross-checked against BLS and Federal Reserve official calendars before run','audit':audit},'source_notes':['HistData M1 bid OHLC; fixed EST UTC-5 source convention; strict contiguous aggregation.','Gross R only; no spread/slippage/commission deduction.','TradingView calendar uses exact historical timestamps; CPI and NFP are exact-title anchors; Fed Interest Rate Decision includes extraordinary decisions if present.'],'errors':[]}
    alltr=[]
    for y in YEARS:
        set_year(y)
        for sym in symbols:
            try:
                ...
                for t in tr:
                    ...
                    a=annotate(t,events,event_days); ann.append(a); alltr.append(a)
    ...
    path=f'wr_{group}_canonical_dayzone_2018_2025.json'
    with open(path,'w') as f: json.dump(out,f,indent=2)
    s=out['summary']
    print('\n===',group.upper(),'CANONICAL T×ZONE 2018-2025 ===')
    print('BASE',s['all_time_baseline'])
    print('FULL',s['canonical_full'],'retention',s['retention'],'years+',s['positive_years'],'/',s['years_with_trade'],'symbols+',s['positive_symbols'],'/',s['symbols_with_trade'])
    print('PRE22',s['pre2022']); print('2022_25',s['y2022_2025'])
